...
- **Keybindings are split global/buffer-local and use a configurable prefix.** Three defcustoms (`leader-key`, `key-prefix`, `key-subprefix`) compose the full prefix (default `C-c g`).  The global command map (8 commands: `S`/`s`/`f`/`n`/`j`/`l`/`c`/`r`) is bound via `global-set-key` on enable.  Buffer-local commands (`d`/`h`/`H` in configured org buffers, `R` in the trash buffer) live in separate minor modes (`buffer-mode`, `trash-mode`) that bind the same prefix to their own keymaps.  Changing the defcustoms via `customize` auto-rebinds via a `:set` handler.  The previous binding at the prefix is saved on enable and restored on disable.

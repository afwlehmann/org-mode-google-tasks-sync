...
22. **Content-hash dedup runs in both modes alongside the ID dedup.** `--collect-content-dupes` groups local synced headings (those with a `GTASK_ID`) by their *computed* canonical hash (`org-mode-google-tasks-sync-org-canonical-hash`, not the stored `:GTASK_CONTENT_HASH:` property — the property may be stale or a placeholder).  For each group with more than one member, the keeper is the newest by `:GTASK_UPDATED:` (last heading in buffer order wins ties, mirroring `puthash` semantics in the ID-dedup); older shadows are trash-snapshotted via `--delete-local` with reason `duplicate-content`.  Prevents the feedback loop where two server-side tasks with identical content (e.g. a duplicate Nebenkostenabrechnung) pull/push each other every tick.  Runs in both `incremental` and `full` modes, mirroring the duplicate-ID dedup (invariant 18) and hidden-task removal (invariant 13) precedent: data-integrity fixes run regardless of mode.
23. **Tie repair retries once with a different `previous` when Google returns the same position.** `--repair-tie-move` compares the move response's `position` to the heading's current `:GTASK_POSITION:`; when they're equal (Google placed the task where it already was — common when the second duplicate is already right after the first), it retries once with `previous=<pre-pair-id>` (the synced sibling before the duplicate run) or with `previous` omitted (move to the front of the list) when `pre-pair-id` is nil.  If the retry also returns the same position, the queue advances (the stable sort keeps duplicates adjacent — no flap).  This converges ties that the pre-0.6.3 single-move approach couldn't fix.  `--collect-tie-pairs-one-level` captures the triple `(ID FIRST-DUP-ID PRE-PAIR-ID)` so the retry anchor is available without re-scanning the buffer.  In a 3+-way tie, every sibling past the first uses the *first* of the run as `FIRST-DUP-ID` so the run converges instead of re-tying.
24. **`--push-new` advances the queue on insert error.** The `:else` callback calls `on-success` (with `nil`) after logging, so `--push-new-queue` advances to the next task even when an insert fails.  Without this, a single insert error would stall the queue: `done` (and therefore `--finalize-apply` → drift → tie repair → sort → save) would never run, leaving the buffer un-repaired and un-saved.  The failed task stays without a `:GTASK_ID:` and is re-queued on the next tick (it's still in the local set with no ID).  Never swallow the error silently — always log before advancing.
